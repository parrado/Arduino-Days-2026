...
def generate_features(jsonPath):
    ...
    for name in vertical_files:   
        with open(name, 'r') as f:
            aux0 = json.load(f)
            accel=aux0['payload']['values']
            feature=extract_accel_features(accel)
            input_data.extend(feature)  # Append features to the training input
            output_data.extend([[1, 0, 0, 0]] * len(feature))  # Append labels for vertical gesture

    for name in sacudir_files:   
        with open(name, 'r') as f:
            aux0 = json.load(f)
            accel=aux0['payload']['values']
            feature=extract_accel_features(accel)
            input_data.extend(feature)  # Append features to the training input
            output_data.extend([[0, 1, 0, 0]] * len(feature))  # Append labels for sacudir gesture

    for name in giro_files:   
        with open(name, 'r') as f:
            aux0 = json.load(f)
            accel=aux0['payload']['values']
            feature=extract_accel_features(accel)
            input_data.extend(feature)  # Append features to the training input
            output_data.extend([[0, 0, 1, 0]] * len(feature))  # Append labels for giro gesture

    for name in quieto_files:   
        with open(name, 'r') as f:
            aux0 = json.load(f)
            accel=aux0['payload']['values']
            feature=extract_accel_features(accel)
            input_data.extend(feature)  # Append features to the training input
            output_data.extend([[0, 0, 0, 1]] * len(feature))  # Append labels for quieto gesture
